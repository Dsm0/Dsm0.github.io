...
let maxFrames = 30; // Maximum number of frames to store
let motionFrameOffset = 5; // Default: compare with 5th previous frame
...
function findBlobs() {
  ...
  // For motion detection, prepare the difference image
  let diffImage = null;
  if (trackingMode === 'motion' && frames.length > motionFrameOffset) {
    diffImage = createImage(width, height);
    diffImage.loadPixels();
    frames[0].loadPixels();
    frames[motionFrameOffset].loadPixels();
    
    // Calculate difference between current and offset frame
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4;
        
        // Get pixel from current frame
        const r1 = frames[0].pixels[i];
        const g1 = frames[0].pixels[i + 1];
        const b1 = frames[0].pixels[i + 2];
        
        // Get pixel from offset frame
        const r2 = frames[motionFrameOffset].pixels[i];
        const g2 = frames[motionFrameOffset].pixels[i + 1];
        const b2 = frames[motionFrameOffset].pixels[i + 2];
        
        // Calculate difference
        const diff = Math.abs(r1 - r2) + Math.abs(g1 - g2) + Math.abs(b1 - b2);
        
        // Set difference image pixel
        diffImage.pixels[i] = diff;
        diffImage.pixels[i + 1] = diff;
        diffImage.pixels[i + 2] = diff;
        diffImage.pixels[i + 3] = 255;
      }
    }
  ...
  for (let y = 0; y < height; y += step) {
    for (let x = 0; x < width; x += step) {
      ...
      } else if (trackingMode === 'motion' && diffImage !== null) {
        // Check if pixel shows significant motion
        const diffValue = diffImage.pixels[index];
        matchesTarget = diffValue > motionThreshold;
      }
...
function drawBlobs() {
  ...
  if (trackingMode === 'color') {
    ...
    if (frames.length > motionFrameOffset) {
...
// Function to save current frame set
function saveFrameSet() {
  // Save the current frame and the comparison frame
  if (frames.length > motionFrameOffset) {
    save(frames[0], 'current-frame.png');
    save(frames[motionFrameOffset], `frame-minus-${motionFrameOffset}.png`);
