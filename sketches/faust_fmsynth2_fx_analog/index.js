// Set to > 0 if the DSP is polyphonic
const FAUST_DSP_VOICES = 0;

// Declare faustNode as a global variable
let faustNode;

// Create audio context activation button
/** @type {HTMLButtonElement} */
const $buttonDsp = document.getElementById("button-dsp");

// Create audio context
const AudioCtx = window.AudioContext || window.webkitAudioContext;
const audioContext = new AudioCtx({ latencyHint: 0.00001 });

// Activate AudioContext and Sensors on user interaction
$buttonDsp.disabled = true;
let sensorHandlersBound = false;
$buttonDsp.onclick = async () => {

    // Import the requestPermissions function
    const { requestPermissions } = await import("./create-node.js");

    // Request permission for sensors
    await requestPermissions();

    // Activate sensor listeners
    if (!sensorHandlersBound) {
        await faustNode.startSensors();
        sensorHandlersBound = true;
    }

    // Activate or suspend the AudioContext
    if (audioContext.state === "running") {
        $buttonDsp.textContent = "Suspended";
        await audioContext.suspend();
    } else if (audioContext.state === "suspended") {
        $buttonDsp.textContent = "Running";
        await audioContext.resume();
        if (FAUST_DSP_VOICES) play(faustNode);
    }
}

// Called at load time
(async () => {

    const { createFaustNode, connectToAudioInput } = await import("./create-node.js");

    const play = (node) => {
        node.keyOn(0, 60, 100);
        setTimeout(() => node.keyOn(0, 64, 100), 1000);
        setTimeout(() => node.keyOn(0, 67, 100), 2000);
        setTimeout(() => node.allNotesOff(), 5000);
        setTimeout(() => play(node), 7000);
    }

    // Create Faust node
    const result = await createFaustNode(audioContext, "FMSynth2_FX_Analog", FAUST_DSP_VOICES);
    faustNode = result.faustNode;  // Assign to the global variable
    if (!faustNode) throw new Error("Faust DSP not compiled");

    // Connect the Faust node to the audio output
    faustNode.connect(audioContext.destination);

    // Connect the Faust node to the audio input
    if (faustNode.getNumInputs() > 0) {
        await connectToAudioInput(audioContext, null, faustNode, null);
    }

    // Create Faust node activation button
    $buttonDsp.disabled = false;

    // Set page title to the DSP name
    document.title = name;

    // After the IIFE, add the slider handlers:

    // Helper function to create slider handlers
    const createSliderHandler = (sliderId, valueId, paramPath) => {
        const slider = document.getElementById(sliderId);
        const valueDisplay = document.getElementById(valueId);
        
        slider.addEventListener('input', () => {
            const value = parseFloat(slider.value);
            valueDisplay.textContent = value.toFixed(2);
            if (faustNode) {
                faustNode.setParamValue(paramPath, value);
            }
        });
    };

    // Create handlers for all sliders
    createSliderHandler('slider-a', 'value-a', '/FMSynth2_FX_Analog/A');
    createSliderHandler('slider-dr', 'value-dr', '/FMSynth2_FX_Analog/DR');
    createSliderHandler('slider-s', 'value-s', '/FMSynth2_FX_Analog/S');
    createSliderHandler('slider-bend', 'value-bend', '/FMSynth2_FX_Analog/bend');
    createSliderHandler('slider-feedb', 'value-feedb', '/FMSynth2_FX_Analog/feedb');
    createSliderHandler('slider-freq', 'value-freq', '/FMSynth2_FX_Analog/freq');
    createSliderHandler('slider-gain', 'value-gain', '/FMSynth2_FX_Analog/gain');
    createSliderHandler('slider-ratio', 'value-ratio', '/FMSynth2_FX_Analog/ratio');

    // Add reset button handlers
    document.querySelectorAll('.reset-button').forEach(button => {
        button.addEventListener('click', () => {
            const sliderId = button.dataset.slider;
            const defaultValue = parseFloat(button.dataset.default);
            const slider = document.getElementById(`slider-${sliderId}`);
            const valueDisplay = document.getElementById(`value-${sliderId}`);
            
            slider.value = defaultValue;
            valueDisplay.textContent = defaultValue.toFixed(2);
            if (faustNode) {
                faustNode.setParamValue(`/FMSynth2_FX_Analog/${sliderId.toUpperCase()}`, defaultValue);
            }
        });
    });

    // Add reset all button handler
    const resetAllButton = document.getElementById('reset-all-button');
    resetAllButton.addEventListener('click', () => {
        document.querySelectorAll('.reset-button').forEach(button => {
            const sliderId = button.dataset.slider;
            const defaultValue = parseFloat(button.dataset.default);
            const slider = document.getElementById(`slider-${sliderId}`);
            const valueDisplay = document.getElementById(`value-${sliderId}`);
            
            slider.value = defaultValue;
            valueDisplay.textContent = defaultValue.toFixed(2);
            if (faustNode) {
                faustNode.setParamValue(`/FMSynth2_FX_Analog/${sliderId.toUpperCase()}`, defaultValue);
            }
        });
    });

})();

const gateButton = document.getElementById('gate-button');

gateButton.addEventListener('mousedown', () => {
    if (faustNode) {
        faustNode.setParamValue("/FMSynth2_FX_Analog/gate", 1);
    }
});

gateButton.addEventListener('mouseup', () => {
    if (faustNode) {
        faustNode.setParamValue("/FMSynth2_FX_Analog/gate", 0);
    }
});

// Add touch support for mobile devices
gateButton.addEventListener('touchstart', (e) => {
    e.preventDefault(); // Prevent default touch behavior
    if (faustNode) {
        faustNode.setParamValue("/FMSynth2_FX_Analog/gate", 1);
    }
});

gateButton.addEventListener('touchend', (e) => {
    e.preventDefault(); // Prevent default touch behavior
    if (faustNode) {
        faustNode.setParamValue("/FMSynth2_FX_Analog/gate", 0);
    }
});
