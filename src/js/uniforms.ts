...
function u_pagePositionGet(shaderObj) {
  const uniform = {
    ...
    }, //these two functions are for debugging puroses more than anything
    type: "2f",
    genValue: function () {
      let pos = uniform.getValue();
...
function u_window_resolutionGet(shaderObj) {
  const uniform = {
    ...
    getLocation: function () {
      return shaderObj.gl.getUniformLocation(
        shaderObj.program,
        "u_window_resolution"
      );
    },
