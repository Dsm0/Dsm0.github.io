...
function moveElemByMouse(e, id, pivotX, pivotY, xfactor = 1, yfactor = 1) {
  ...
  move(
    elem,
    pivotX + (pivotX - e.clientX) * xfactor,
    pivotY + (pivotY - e.clientY) * yfactor
  );
}

function moveElemByMouseMax(
  e,
  id,
  pivotX,
  pivotY,
...
function moveByPivot(e: MouseEvent, canv) {
  ...
  let maxX: number =
    canv.getAttribute("data-maxX") != null
      ? xpercentToFloat(canv.getAttribute("data-maxX"))
      : 0;

  let maxY: number =
    canv.getAttribute("data-maxY") != null
